...
`exampleData` is a list containing 3 elements:
  ...
  - `exampleData$genotypes`: `data.frame` containing the genotypic data
    ...
    pairs.
  - `exampleData$genoMap`: `data.frame` containing the coordinates of
    the 3333 individuals’ markers. This data.frame contains 3 columns:
    `chr`, `pos` and `lociNames`.
  - `exampleData$lociEffects`: `numeric` vector containing the “true”
...
Let’s load these information (stored in `exampleData$genoMap`) and
create the `lociInfo` object.
...
myLoci <- lociInfo$new(genoMap = exampleData$genoMap, specie = specie_statEx)
print(SNPs)
...
initPop <- createPop(geno = exampleData$genotypes, lociInfo = myLoci, popName = "Initial population")
```
...
(selectedInds <- selectBV(pop = initPop, lociEffects = exampleData$lociEffects, n = 10))
